...
export async function handlePrefix(message: any, guildId: string, args: string[]): Promise<void> {
  ...
  if (!action) {
    return showCurrentPrefix(message, guildId);
  }
  ...
  switch (action) {
    case 'set':
      return setPrefix(message, guildId, args.slice(1).join(' '));
    case 'reset':
      return resetPrefix(message, guildId);
    default:
      await message.reply(t(`${localeKey}.unknownAction`, { action }));
  }
}

async function showCurrentPrefix(message: any, guildId: string): Promise<void> {
  const config = await guildConfigService.get(guildId);
  const prefix = config.prefix ?? '!';

  if (config.prefix) {
    await message.reply(t(`${localeKey}.currentCustom`, { prefix }));
  } else {
    await message.reply(t(`${localeKey}.currentDefault`, { prefix }));
  }
}

async function setPrefix(message: any, guildId: string, value: string): Promise<void> {
  ...
  await message.reply(t(`${localeKey}.setSuccess`, { prefix: raw }));
}

async function resetPrefix(message: any, guildId: string): Promise<void> {
  await guildConfigService.set(guildId, { prefix: null });
  await message.reply(t(`${localeKey}.resetSuccess`));
}
